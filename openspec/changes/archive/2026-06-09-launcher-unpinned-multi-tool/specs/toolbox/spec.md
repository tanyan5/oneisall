...
Opening a tool from the **launcher overlay** SHALL always clear window pin mode before the tool view is shown. The main window SHALL open in the standard unpinned immersive layout regardless of prior pin state or whether the selected tool is the same as the currently active tool.
...
- **WHEN** user invokes a global shortcut for a different tool while another tool is open in pin mode
- **THEN** the requested tool opens without pin chrome, without taskbar presence, and with the plugin's own top bar

#### Scenario: Launcher open always unpinned

- **WHEN** user had pin mode active and selects any toolbox plugin from the launcher overlay
- **THEN** pin mode is cleared and the selected tool opens in unpinned immersive layout

#### Scenario: Launcher opens different tool while pinned

- **WHEN** user has plugin A open in pin mode and selects plugin B from the launcher overlay
- **THEN** the main window shows plugin B in unpinned layout and pin chrome for plugin A is removed
