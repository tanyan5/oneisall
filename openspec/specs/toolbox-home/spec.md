# toolbox-home Specification

## Purpose

The Management Center is the main-window browsing shell for installed toolbox plugins and Shankai applications. Users enter from the launcher jellyfish control to discover announcements, welfare content, recommendations, and on-demand plugin previews before opening immersive tool views.

## Requirements

### Requirement: Management center shell

The application SHALL provide a **Management Center** shell (entered from the launcher jellyfish control). The shell SHALL use a **two-column** layout: a **left** column listing installed toolbox tools (and searchable Shankai applications when filtering), and a **right** main content area.
...
The system SHALL supply announcement items for the Management Center hub. Version 1 SHALL read from local JSON (`resources/announcements.json` or user-data override). Users SHALL be able to dismiss an announcement (`dismissedAnnouncementIds` in settings).
...
The system SHALL provide a **PromoProvider** for the welfare/partnership hub card. Version 1 SHALL load from local `promo.json`. Version 2 SHALL support remote HTTPS fetch with user-data cache fallback. The renderer SHALL obtain promo data via main-process IPC only. Users SHALL be able to disable promo display in settings and dismiss individual promos.
